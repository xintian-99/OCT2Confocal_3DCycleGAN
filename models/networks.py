...
#         else:
#             raise ValueError("Padding must be an integer or a 3-element tuple.")
#
#     def forward(self, x):
#         return F.pad(x, (self.padding[2], self.padding[2], self.padding[1], self.padding[1], self.padding[0], self.padding[0]), mode='reflect')
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    elif netG == 'selfatten_unet':
        net = UNet(input_nc, output_nc,256)
    else:
...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, input):
        """Standard forward"""
        # x = input
        # for i, sub_module in enumerate(self.model):
        #     x = sub_module(x)
        #     print(f"After layer {i} ({type(sub_module).__name__}): {x.shape}")
